# A05ClassPrH.py
# Local helper module with two simple record-like classes:
# - House: rows from a House elections dataset
# - President: rows from a Presidential elections dataset
#
# Notes:
# - Accessors are prefixed with get_*, mutators with set_*.
# - __str__ returns a single comma-separated line ending with ". ".
#   Values are stringified safely (int/None won't crash).
...
class House:
  ...
  def set_state_ic(self, state_cen):
    # Note: parameter name kept as in original (state_cen) for compatibility.
  ...
  def __str__(self):
    # Safe stringification of all fields, preserving order
    vals = [
      self.__year, self.__state, self.__state_po, self.__state_fips,
      self.__state_cen, self.__state_ic, self.__office, self.__district,
      self.__stage, self.__runoff, self.__special, self.__candidate,
      self.__party, self.__writein, self.__mode, self.__candidatevotes,
      self.__totalvotes, self.__unofficial, self.__version
    ]
...
class President:
  ...
  def __str__(self):
    ...
    vals = [
      self.__year, self.__state, self.__state_po, self.__state_fips,
      self.__state_cen, self.__state_ic, self.__office, self.__candidate,
      self.__party, self.__writein, self.__candidatevotes, self.__totalvotes,
      self.__version, self.__notes
    ]
    return ", ".join("" if v is None else str(v) for v in vals) + ". "
